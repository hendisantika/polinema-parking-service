...
public class KantongController {
    ...
    @GetMapping
    public KantongDto getMahasiswaDto(@PathVariable("nim") String nim) throws Exception {
        try {
            Kantong kantong = repository.findById(nim).orElseThrow(() -> new NotFoundException(nim));
